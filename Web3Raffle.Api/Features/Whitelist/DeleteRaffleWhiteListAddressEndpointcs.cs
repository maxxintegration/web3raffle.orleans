...
public class DeleteRaffleWhiteListAddressEndpointcs : Endpoint<Web3RaffleWhitelistRequestModel, EmptyResponse>
{
	private readonly IClusterClient orleansClient;

	public DeleteRaffleWhiteListAddressEndpointcs(IClusterClient orleansClient)
	{
	...
	{
		this.Delete("/raffles/{raffleId}/whitelist/{walletAddress}");
		//this.AllowAnonymous();
		this.Roles("Admin");
	...
	public override async Task HandleAsync(Web3RaffleWhitelistRequestModel req, CancellationToken ct)
	{
		...
		var whitelist = await grain.GetWhitelistAsync(req.RaffleId, req.WalletAddress, ct.ToGrainCancellationToken());

		req.Data = new List<Web3RaffleWhitelistModel>();
		if (whitelist != null)
			req.Data.Add(whitelist);
